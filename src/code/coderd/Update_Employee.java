...
class Update_Employee extends Add_Employee implements ActionListener{
    ...
    Update_Employee(String idaa){
        super(0);
        f=new JFrame("Employee Management System - Update Employee Details");
        f.setVisible(true);
        ...
        id15.setLayout(null);
//        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("code/icons/add_employee.jpg"));
//        id15.setIcon(img);

        id8 = new JLabel("Update Employee Detail:");
        id8.setBounds(215,10,600,50);
        id8.setFont(new Font("poppins",Font.BOLD,40));
...
//                t9.setText(rs.getString(3));
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
                DatabaseConnection con = new DatabaseConnection();
                String str = "update employee set name='"+t1.getText()+"',fname='"+t2.getText()+"',age='"+age+"',dob='"+dat+"',address='"+t3.getText()+"',phone='"+t4.getText()+"',email='"+t5.getText()+"',education='"+t6.getText()+"',post='"+t7.getText()+"',ctzn_no='"+t8.getText()+"',emp_id='"+t9.getText()+"' where emp_id='"+id_emp+"'";
